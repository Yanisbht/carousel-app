...
public class CarouselService {
    ...
    public String generateFootball(String style) throws Exception {
        String prompt = "Genere une phrase virale courte pour un compte football TikTok. Style : sobre, poetique, universel. MAX 10 MOTS. Retourne UNIQUEMENT ce JSON sans backticks : {\"hashtags\":[\"football\",\"foot\",\"soccer\",\"footballmotivation\",\"fyp\",\"tiktok\"],\"slides\":[{\"type\":\"football\",\"phrase\":\"phrase virale MAX 10 MOTS\"}]}";
        return callGemini(prompt);
    }

    // ================================================================
    // PHILO 2026 — mis a jour : retourne philosophe + sujet + teaser
    // Le meme philosophe et sujet restent sur les 3 slides.
    // Seul le teaser change (accroche, citation, chute).
    // Le composant frontend affiche : PHILOSOPHE / FACE A / SUJET
    // et le teaser en dessous comme description.
    // ================================================================
    public String generatePhilo2026(String philosophe, String sujet) throws Exception {
        String prompt = "TACHE : genere un carrousel TikTok 3 slides pour le couple : " + philosophe + " face a " + sujet + ". Tu dois IMPERATIVEMENT utiliser " + philosophe + " et " + sujet + " -- pas autre chose. "
            + "STYLE : humour philosophique, intelligent, comme ces exemples parfaits : "
            + "(1) Socrate / Tinder -- teaser slide1: Et toi, te connais-tu vraiment ? -- teaser slide2: Il questionnait les pretendants. Tinder aussi finalement. -- teaser slide3: Au moins lui, il attendait les vraies reponses. "
            + "(2) Diogene / Amazon -- teaser slide1: Je vivais dans un tonneau. Vous, dans des cartons. -- teaser slide2: Il rejetait le superflu. Amazon vous le livre en 24h. -- teaser slide3: Au moins le mien me coutait rien. "
            + "(3) Nietzsche / LinkedIn -- teaser slide1: Ils creent des dieux qu ils appellent recruteurs. -- teaser slide2: Il voulait des surhommes. LinkedIn veut des profils optimises. -- teaser slide3: Au moins le sien avait de la volonte. "
            + "STRUCTURE DES 3 SLIDES : "
            + "Slide 1 = accroche ironique qui pose le clash philosophe/sujet. MAX 12 MOTS. "
            + "Slide 2 = la comparaison philosophique : ce que le philosophe pensait VS ce que le sujet moderne fait. MAX 12 MOTS. "
            + "Slide 3 = la chute ironique courte, toujours du style 'Au moins [avantage subtil du philosophe vs le moderne]'. MAX 10 MOTS. "
            + "INTERDICTIONS : aucun ..., phrases completes uniquement. Ton : drole mais vrai. "
            + "Retourne UNIQUEMENT ce JSON sans backticks : "
            + "{\"hashtags\":[\"philosophie\",\"humour\",\"2026\",\"culture\",\"tiktok\",\"fyp\"],"
            + "\"slides\":["
            + "{\"type\":\"philo2026\",\"philosophe\":\"" + escapeJson(philosophe) + "\",\"sujet\":\"" + escapeJson(sujet) + "\",\"teaser\":\"accroche ironique MAX 12 MOTS\"},"
            + "{\"type\":\"philo2026\",\"philosophe\":\"" + escapeJson(philosophe) + "\",\"sujet\":\"" + escapeJson(sujet) + "\",\"teaser\":\"comparaison philosophe VS moderne MAX 12 MOTS\"},"
            + "{\"type\":\"philo2026\",\"philosophe\":\"" + escapeJson(philosophe) + "\",\"sujet\":\"" + escapeJson(sujet) + "\",\"teaser\":\"chute ironique Au moins... MAX 10 MOTS\"}"
            + "]}";
        return callGemini(prompt);
    }
    ...
    public String generateOneShot(String style) throws Exception {
        String prompt = "Genere une phrase de vie naturelle et universelle pour TikTok. Style : comme si tu l ecrivais dans un journal intime. Pas de cliche motivationnel. Naturelle, directe, qui fait se reconnaitre. 15 a 25 MOTS. Retourne UNIQUEMENT ce JSON sans backticks : {\"hashtags\":[\"motivation\",\"mindset\",\"lifestyle\",\"fyp\",\"citation\",\"tiktok\"],\"slides\":[{\"type\":\"oneshot\",\"phrase\":\"phrase naturelle 15-25 MOTS\"}]}";
        return callGemini(prompt);
    }
    ...
    private String buildPrompt(String theme, String style, String sujet) {
        ...
        return "Cree un carrousel TikTok 3 slides sur ce theme : " + theme + ". INTERDICTIONS STRICTES : (1) AUCUNE slide ne doit finir par ... ou contenir ... (2) chaque slide est une phrase grammaticalement COMPLETE qui finit par . ou ! ou ? (3) les 3 phrases lues a la suite doivent former un paragraphe coherent." + sujetPart + " Style : simple, direct, reference a la pop culture ou a des faits reels connus. EXEMPLE 1 -- slide1: Walter White a tout perdu en voulant tout controler. slide2: Il controlait rien en fait. slide3: Choisir ce qu on desire change tout. Slide1 reference pop culture ou fait reel MAX 6 MOTS. Slide2 ce que ca revele vraiment MAX 6 MOTS. Slide3 la lecon simple et directe MAX 8 MOTS. Theme : " + theme + ". Retourne UNIQUEMENT ce JSON sans backticks : {\"hashtags\":[\"philosophie\",\"culture\",\"pensee\",\"tiktok\",\"fyp\",\"vrai\"],\"slides\":[{\"type\":\"hook\",\"citation\":\"MAX 6 MOTS\",\"auteur\":\"\",\"origine\":\"\"},{\"type\":\"intrigue\",\"question\":\"MAX 6 MOTS\"},{\"type\":\"lesson\",\"titre\":\"\",\"corps\":\"MAX 8 MOTS\"}]}";
    }
    ...
    private String buildPromptVideo(String transcription, String style) {
        String excerpt = transcription.length() > 1500 ? transcription.substring(0, 1500) : transcription;
        return "Transforme cette transcription en carrousel TikTok 3 slides. INTERDICTIONS STRICTES : (1) AUCUN ... dans les slides. (2) chaque slide est une phrase complete avec . ou ! ou ?. Style : simple, direct, avec des references a la pop culture. Slide1 observation du quotidien MAX 6 MOTS. Slide2 reference pop culture ou image forte MAX 6 MOTS. Slide3 verite simple et directe MAX 8 MOTS. Transcription : " + excerpt + ". Retourne UNIQUEMENT ce JSON sans backticks : {\"hashtags\":[\"philosophie\",\"culture\",\"pensee\",\"vrai\",\"tiktok\",\"fyp\"],\"slides\":[{\"type\":\"video_hook\",\"concept\":\"2 mots\",\"accroche\":\"MAX 6 MOTS\"},{\"type\":\"video_explication\",\"titre\":\"2 MOTS\",\"corps\":\"MAX 6 MOTS\"},{\"type\":\"video_cta\",\"texte\":\"MAX 8 MOTS\",\"question\":\"\"}]}";
    }
